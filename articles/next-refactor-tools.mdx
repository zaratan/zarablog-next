...
date: 2020-11-22T04:33:13+09:00
categories:
  - front
  - tools
  - code
...
En tant que développeur les point les plus importants sont :
...
Seriez-vous capable de me donner la liste des méthodes présentes sur `event.target` et `event.current.target` et être sûr que vous les utilisez avec le bon type d’arguments ? Moi… non. TS, oui. 

Ou encore, cas que j'ai eu tout récemment, un besoin d'utiliser les APIs de chrome pour les extensions.

<Image src="" height="552" width="1702" alt="Chrome extension API autocomplete" />
...
- Prettier n’a pas de règles quant à la qualité de code. Prettier n’a aucun problème à ce que votre code ait des variables inutilisées ou encore des règles de performance/sécurité.
...
module.exports = {
  ...
  rules: {
    ...
    // Les arrow function auront des {} seulement si nécessaire.
    ...
    // notemment les variables commençant par _
    'no-unused-vars': 'off',
