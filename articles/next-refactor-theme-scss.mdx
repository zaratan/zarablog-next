...
On applique notre classe de thème à un element haut dans la hiérarchie du DOM.
...
On défini un bouton pour switch entre les différents thèmes
...
La raison derière ce choix est pour être capable d'acceder au contexte au plus haut possible dans la hiérarchie.

### React: Local Storage

Un utilisateur accédant à notre site pour la première fois, se voit assigner le thème sombre.
Si jamais il choisi de passer au thème clair puis recharge la page on lui sert à nouveau le thème sombre.
...
### React: Detecter le thème préféré de l'utilisateur

Si l'utilisateur est sur un OS qui supporte les thèmes sombres et clair et que son [navigateur supporte sa detection](https://caniuse.com/prefers-color-scheme), on peut :
Si et seulement si il n'a jamais choisi de thème manuellement, respecter cette préférence.

Encore une fois, juste besoin de changer legerement le context:
...
NextJS à 1 particularité par rapport à tout ça :
...
C'est très pratique car ça permet d'assurer que un module CSS ne s'appliquera qu'au component dans lequel il est utilisé et n'aura donc pas d'effets de bord sur le reste de vos pages.

Cependant, certaines fois on ne veut pas que certaines classes soient préfixées dans notre scss (`.theme--light` par exemple).
On doit donc notter ces classes de la manière suivante dans nos fichiers `xxx.module.scss`:
